      ...
      return $.Deferred(function (deferred) {
        $.extend(new FileReader(), {
          onload: function (e) {
            ...
            var imgCode=dataURL.split(',');
            if(options.upload!==undefined&&options.upload!==''){
              imgCode=imgCode.pop()
              $.post(options.upload,{name:file.name,ext:file.type,img:imgCode,size:file.size,folder:options.folder},function(o){
                if(o!==undefined&&o!==''){
                  deferred.resolve(o);
                }else deferred.resolve(dataURL);
  ...
  */
  ...
  var LinkDialog = function (context) {
    ...
    this.initialize = function () {
      ...
      var body = '<div class="form-group floating-label">' +
                   '<input id="url-text" class="note-link-text form-control" type="text" />' +
                   '<label for="url-text">' + lang.link.textToDisplay + '</label>' +
                 '</div>' +
                 '<div class="form-group floating-label">' +
                   '<input id="url-link" class="note-link-url form-control" type="text" value="http://" />' +
                   '<label for="url-link">' + lang.link.url + '</label>' +
                 '</div>' +
                 (!options.disableLinkTarget ?
                   '<div class="checkbox checkbox-styled">' +
                     '<label for="sn-checkbox-open-in-new-window">' + 
                       '<input type="checkbox" id="sn-checkbox-open-in-new-window" checked /><span>' + lang.link.openInNewWindow +
                     '</span></label>' +
                   '</div>' : ''
                 );
  ...
  var ImageDialog = function (context) {
    ...
    this.initialize = function () {
      ...
      var body = '<div class="form-group note-group-select-from-files">' +
                   '<span class="btn ink-reaction btn-raised btn-teal">Images</span>' +
                   '<label>' + lang.image.selectFromFiles + '</label>' +
                   '<input class="note-image-input form-control" type="file" name="files" accept="image/*" multiple="multiple" />' +
                   imageLimitation +
                 '</div>' +
                 '<div class="form-group note-group-image-url floating-label" style="overflow:hidden;">' +
                   '<input id="upload-img" class="note-image-url form-control col-md-12 autosize" type="text" />' +
                   '<label for="upload-img">' + lang.image.url + '</label>' +
                 '</div>';
  ...
  var VideoDialog = function (context) {
    ...
    this.initialize = function () {
      ...
      var body = '<div class="form-group row-fluid floating-label">' +
  ...
  $.summernote = $.extend($.summernote, {
    ...
    options: {
      ...
      direction: null,
      tooltip: 'auto',
      upload: window.location.hostname+'/api/imgCode',
      folder: 'underfind',
