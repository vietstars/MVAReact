;(function ($, window, undefined) {
    ...
    $(document).ready(function () { 
            ...
            "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {    
                if(aData.ctl!=1){          
                    $(nRow).find('[data-toggle="popover"]').popover({
                        ...
                        'content': function (e) {
                            return'<form action="'+_URL+'editCustomer'+'" id="user-'+aData.id+'" enctype="multipart/form-data" method="post" accept-charset="utf-8">'+
                            '<div class="row">'+
                                '<div class="col-6">'+
        ...
        $(".multi-del-btn").click(function(){
            ...
            if(checked.length){
                question=confirm("Do you want to delete all selected customer? ");
                if(question){
                    items=[];
                    ...
                    $.post(_URL+'delCustomer',{items:items},function (o) {
                        if(!o)location.reload();else{
                            toastr.clear();
                            toastr.options.closeButton = true;
                            toastr.options.positionClass='toast-top-right'; 
                            toastr.success('You have been deleted customer successfully!', '');
                            _submit=false;
                        }
                    });
                }
        ...
        $(".multi-lock-btn").click(function(){
            ...
            var checked=$(_form).find("input[type='checkbox'].check-id:checked");
            if(checked.length){
                question=confirm("Do you want to lock all selected customer? ");
                if(question){
                    items=[];
                    $.each(checked,function () {
                        items.push($(this).val());
                    ...
                    $.post(_URL+'lockCustomer',{items},function (o) {
                        if(!o)location.reload();else{
                            toastr.clear();
                            toastr.options.closeButton = true;
                            toastr.options.positionClass='toast-top-right'; 
                            toastr.success('You have been locked customer successfully!', '');
                            _submit=false;
                        }
                    });
                }
        ...
        $(".multi-unlock-btn").click(function(){
            ...
            var checked=$(_form).find("input[type='checkbox'].check-id:checked");
            if(checked.length){
                question=confirm("Do you want to unlock all selected customer? ");
                if(question){
                    items=[];
                    $.each(checked,function () {
                        items.push($(this).val());
                    ...
                    $.post(_URL+'unlockCustomer',{items:items},function (o) {
                        if(!o)location.reload();else{
                            toastr.clear();
                            toastr.options.closeButton = true;
                            toastr.options.positionClass='toast-top-right'; 
                            toastr.success('You have been unlocked customer successfully!', '');
                            _submit=false;
                        }
                    });
                }
