...
class Customer extends CI_Controller {
		...
		if($_POST){
			...
			if($results){
				foreach ($results as $user) {
					...
					$data=array(
						"id"=>$user->id,
						"text-name"=>"<div class='checkbox'><input type='checkbox' class='check-id' value='".$user->id."'><label></label></div><div class='img-avatar pull-right' style='background:url(".$this->load->module('image')->img($user->avatar)->folder('avatar')->getThumb().");'></div><a href='javascript:;'  data-toggle='popover'><strong data-toggle='tooltip' data-placement='top' data-original-title='Click to view customer'>".$user->fullname."</strong></a> [<i class='text-danger la la-".$user->gender."'></i>]<br>".$user->email,
	...
	public function editCustomer()
	{
        ...
        {	
			...
			if ($this->form_validation->run()){
	            ...
	            ->where(array('id'=>$id))
				->update('user');
				$this->session->set_flashdata('error', "You have been updated customer information successfully!");
				redirect(_URL.'dashboard/customer');
			}	
		}		
		$this->index();
	}
	public function newCustomer()
	{
        ...
        {	
			...
			$this->session->set_flashdata('error', "You have been added new customer successfully!");
			redirect(_URL.'dashboard/customer');	
		}
		$this->index();
	}
